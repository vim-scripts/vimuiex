...
"if vxlib#plugin#StopLoading('_plugin_autogen_0b325a:68d5')
"   finish
"endif
...
" START Plugin
if s:ContinueLoading('vxlib#quickfixpreview')
 if !(has('windows')&&has('quickfix'))
   call s:SetLoaded('vxlib#quickfixpreview', -2)
   let g:VxPluginMissFeatures['vxlib#quickfixpreview'] = 'windows&&quickfix: ' . s:StrHas('quickfix,windows')
 else
  try
   autocmd FileType qf call vxlib#cmd#PrepareQFixPreview()
   call s:SetLoaded('vxlib#quickfixpreview', 1)
  catch /.*/
   call s:SetLoaded('vxlib#quickfixpreview', -9)
   call s:Exception(v:throwpoint, v:exception, 'vxlib#quickfixpreview')
  endtry
 endif
endif
" END Plugin
...
if s:ContinueLoading('vimuiex#vxjump')
 if !(has('python')&&has('python_screen'))
   ...
   let g:VxPluginMissFeatures['vimuiex#vxjump'] = 'python&&python_screen: ' . s:StrHas('python,python_screen')
 else
  try
...
" Source: vimuiex/vxlist.vim
" START Plugin
if s:ContinueLoading('vimuiex#vxlist#init')
  try
   call s:CheckSetting('g:VxPopupListPos', '{}')
   call s:CheckSetting('g:_VxPopupListPosDefault', '{}')
   call s:SetLoaded('vimuiex#vxlist#init', 1)
  catch /.*/
   call s:SetLoaded('vimuiex#vxlist#init', -9)
   call s:Exception(v:throwpoint, v:exception, 'vimuiex#vxlist#init')
  endtry
endif
" END Plugin
